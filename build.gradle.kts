...
val moduleLwjgl: String by project

val binExtract: String by project
val binGenerator: String by project
val binJavadoc: String by project
val binLwjgl: String by project
val binSamples: String by project
val binTemplates: String by project
val binTest: String by project
val binTestHtml: String by project

tasks.register("clean-kotlin") {
    description = "Deletes Java binary files produced by this script"
    doLast {
        delete(binGenerator)
        delete(binTemplates)
    }
...
tasks.register("clean-java") {
    description = "Deletes Kotlin binary files produced by this script"
    doLast {
        delete(binLwjgl)
        delete(binSamples)
        delete(binTest)
...
tasks.register("clean-native") {
    description = "Deletes native binary files produced by this script"
}

tasks.register("clean-generated") {
    description = "Deletes sources, test results and documentation generated by this script"
    dependsOn("clean-generated-java", "clean-generated-native")
    doLast {
        delete(binTestHtml)
        delete(binJavadoc)
    }
...
tasks.register("clean-generated-java") {
    description = "Deletes Java sources generated by this script"
    doLast {
        delete("$binGenerator/generated-touch.txt")
        subprojects
            .forEach {
                delete("$moduleLwjgl/${it.name}/src/generated/java")
            }
    }
}

tasks.register("clean-generated-native") {
    description = "Deletes native sources generated by this script"
    doLast {
        delete("$binGenerator/generated-touch.txt")
        subprojects
            .forEach {
                delete("$moduleLwjgl/${it.name}/src/generated/c")
            }
    }
}

tasks.register("all") {
    description = "Builds LWJGL and runs the tests"
    dependsOn("compile-templates", "tests")
}

tasks.register("-compile-generator") {
    doLast {
        logger.info("Compiling Kotlin generator...")
        File(binGenerator).mkdirs()
        this.finalizedBy(
            subprojects
            .first { it.name == "generator" }
